module FeatureHelpers
  ...
  def post_prototype(prototype)
    ...
    attach_file(
      'prototype[captured_images_attributes][0][content]', Rails.root.join('spec', 'fixtures', 'images', 'prototype.jpg'), visible: false
    )
    3.times do |i|
      attach_file(
        "prototype[captured_images_attributes][#{i + 1}][content]", Rails.root.join('spec', 'fixtures', 'images', 'prototype.jpg'), visible: false
      )
    end
    click_on 'Publish'
  end
